...
| アーティスト経由生成(管理画面・iOS → `POST /api/generate` + `artistSongId`) | `artist` | なし(要素プール自体を提示しない) | なし | 登録アーティストの曲を「リファレンス楽曲」として提示し、その音楽的特徴を英語の音楽用語に翻訳した style を書かせる。固有名詞は禁止(Suno のモデレーション対策)、歌詞は原曲を複製しない完全な新作 |
