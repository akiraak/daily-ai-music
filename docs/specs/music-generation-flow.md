...
| アーティスト経由生成(管理画面・iOS → `POST /api/generate` + `artistSongId`) | `artist` | なし(要素プール自体を提示しない) | なし | 登録済みの曲を「リファレンス楽曲」として提示し、その音楽的特徴を英語の音楽用語に翻訳した style を書かせる。固有名詞は禁止(Suno のモデレーション対策)、歌詞は原曲を複製しない完全な新作。参照曲を登録する入口はアーティスト名からと曲名から(逆引き)の 2 つ(下記) |
...
| リファレンス楽曲+作り方 | 参照曲のアーティスト・曲名・アルバム・リリース年・ジャンルと、音楽的特徴を英語の音楽用語へ翻訳する指示・固有名詞の禁止・原曲歌詞の複製禁止・`intent` に根拠を書く指示。自由テキストがあれば「追加の要望」として併記 | artist のみ |
