...
**取り込みは既定で無効にする**ことにした(当初は「ノイズ判定に当たる曲だけ無効・残りは有効」)。
1 アーティスト最大 200 曲を機械的に取り込む以上、参照曲にする曲は人が選ぶのが素直で、
「Live / Remix だけ外す」では大半の使わない曲が候補に残り続けるため。

この変更で以下が本文から変わっている。

- `insertArtistSongs()` は曲名を見ない(常に既定の `enabled = 0` で入る)
- `server/src/songtitle.ts`(ノイズ判定の移設先)は不要になり削除
- 既存データの移行は「ノイズな曲を無効に」ではなく **全曲を無効に**
  (実施記録は `settings` の `migration_disable_all_songs`)
- 導入直後は有効な曲が 0 件になるため、曲を有効にするまで毎日の自動生成は止まる
  (スケジューラは 30 分ごとに再試行し、`POST /api/daily/run` は 409)
