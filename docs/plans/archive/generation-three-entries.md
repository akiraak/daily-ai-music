...
- [x] Phase 2: iOS — 生成タブの 3 経路化・`ArtistPickerView`・改名(2026-08-12 完了。3 経路は丸地アイコン + 太字タイトル + 説明の同形行。おまかせはヒーロー + 即実行ボタンから行タップ → 確認ダイアログに変更。UI テスト 5 本成功 — 確認ダイアログはポップオーバー表示でキャンセル行が無いことがあるため外側タップで閉じる)
