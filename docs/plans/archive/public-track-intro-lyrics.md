...
公開ページ(2026-08-12 公開。[public-tracks-web](archive/public-tracks-web.md))の曲詳細は、カバー・曲名・日付・生成モデル名とプレイヤーだけで、**曲の中身の手がかりが無い**。聴く前に「どんな曲か」が分かるように、詳細ページへ **短い紹介** と **歌詞** を足す。紹介は**トップの一覧カードにも短く(2 行まで)出す** — 一覧の段階で曲を選べるようにするため。
...
| 詳細の取得 | **`GET /site/api/tracks/:id` を新設**。歌詞は詳細だけで返す | 歌詞は 1 曲で数百〜数千字。全曲を返す一覧に載せると一覧ページの転送量が跳ねる |
| 一覧ページ(トップ) | **カードにも紹介を短く出す**(2 行までのクランプ)。一覧 API `GET /site/api/tracks` にも `intro` を含める | 一覧の段階で「どんな曲か」の手がかりが要る。intro は 60〜100 字なので 36 曲でも数 KB で、歌詞と違って一覧に載せても転送量の問題にならない |
| 曲詳細の「ほかの曲」 | 変更しない(カバー・曲名・日付のまま) | 回遊用の小さいカードなので、文章を足すと主役の一覧と区別がつかなくなる |
...
- **歌詞を一覧 API に含めてしまう**: 実装は一番小さいが、一覧の転送量が 36 曲で数十 KB → 数百 KB に増える。曲は毎日増えるので後で必ず効いてくる(intro は 1 曲 100 字程度なので一覧に載せる)
- **一覧では紹介を全文出す**: 文量が曲ごとにばらつき、カードの高さが不揃いになってグリッドが乱れる。2 行までに切って、続きは詳細ページで読ませる
...
- 一覧の `publicTrackJson()` に `intro` を足す(一覧・詳細で同じ値。**歌詞は詳細だけ**)
...
- `list.js`: カードの `.models` の下に `.intro` を足す(`intro` が無い曲は要素ごと出さない)
- `style.css`:
  - 詳細 — 紹介は本文よりやや大きめ、歌詞は `white-space: pre-line` で行間広め。モバイル幅で読める字送りにする
  - 一覧 — `.card .intro` は `-webkit-line-clamp: 2` の 2 行クランプ(`display: -webkit-box` + `overflow: hidden`)。文量のばらつきでカードの高さが揃わなくなるのを防ぐ。モバイル(2 列)では字を一段小さくして 2 行のまま
...
- デプロイ後にバックフィルを実行(まず `--dry-run --limit 3` で文面を見る):
  ...
  ```
- 確認: `/track/:id` に紹介と歌詞が出る / トップの一覧カードに紹介が 2 行で出る / 公開 API に参照曲・スタイル・狙いが含まれない / 一覧 API に歌詞が入っていない / 非公開の曲は 404 のまま / `/api/*` の認証が緩んでいない
...
- `server/site/track.html` / `track.js` — 詳細の紹介・歌詞の表示
- `server/site/list.js` / `style.css` — 一覧カードの紹介(2 行クランプ)
...
- 隔離 DB テストサーバー(`sqlite3 .backup` のコピー + `daily_enabled=false`)で:
  ...
  - 一覧 `GET /site/api/tracks` に `intro` が入り、**`lyrics` は入らない**こと
  - 非公開にした曲の id が 404、数字でない id が 404
  - `stripLyricSectionTags()` の単体確認(タグ行の除去 / 連続空行の畳み込み / タグの無い歌詞をそのまま返す)
  - インストゥルメンタル(歌詞が空)の曲で `lyrics` が null になり、ページに歌詞ブロックが出ないこと
- 公開ページはヘッドレス Chrome で PC / モバイル幅のスクリーンショット確認(**一覧は紹介が 2 行で切れてカードの高さが揃うこと**・紹介が無い曲が混ざっても崩れないこと)
...
- [ ] Phase 1: サーバー — `tasks.intro` + 一覧 API に intro + `GET /site/api/tracks/:id`(intro + タグ除去済み歌詞)
- [ ] Phase 2: 公開ページに表示(曲詳細は紹介・歌詞、トップの一覧カードは紹介 2 行 + 管理画面に intro の表示)
