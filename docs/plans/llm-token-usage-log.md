...
- [x] Phase 2: 本番反映(2026-08-12 完了。進行中の生成が無いことを確認してから通常フローで再起動。`/api/tracks` に `llmInputTokens` 等が出ること・旧データが null なことを確認)
- [ ] 2026-08-13 の毎日の自動生成(PT 6:00)後に、実際の値が記録されていることを確認(実生成での確認はコストが掛かるため翌朝の定期生成で行う)
