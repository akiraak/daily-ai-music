...
- 管理画面のページ: 楽曲一覧(`index.html`)/ アーティスト(`artists.html`。アーティスト名または曲名で検索して登録・曲一覧・曲を選んで生成)/ 設定(`settings.html`。歌声の言語・生成モデルの表示と effort の切り替え・毎日の自動生成・外部コンテキスト)
