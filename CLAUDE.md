...
- **ログ解析(`/logs`)が見つけた修正・改善項目は `FIXES.md` で管理する**(`TODO.md` には入れない)。実施したら削除せず `[x]` にする(`/logs` が台帳 `docs/error-triage.md` の「修正済み」化に使う)。着手時は通常の作業着手ルールに乗せる
