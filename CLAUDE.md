...
- **`src/llm.ts`**: Claude API(既定 `claude-sonnet-5`、`.env` の `LLM_MODEL` で変更可)。生成リクエストからスタイル・英語歌詞・日本語訳・タイトル・狙い・使用プリセット・参照した情報源を構造化出力で生成し、Suno へ `customMode: true` で送信する。**参照曲があるときだけ `web_search` サーバーツールを有効にして実際の曲情報(BPM・キー・コード進行・編成・ボーカル)を調べさせる**(判定はモード名ではなく `referenceSong` の有無)。`.env` に `ANTHROPIC_API_KEY` 必須(起動時 fail-fast)
