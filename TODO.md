...
- [ ] リアルワード: 外部リソース(ニュース・天気)から曲の着想ワードを作り、保存・使用制限で似た曲の連続生成を防ぐ(同一ワードは直近 30 日で 2 回まで、古いワードは自動で再利用可能に)[plan](docs/plans/real-world-words.md)
  ...
  - [ ] Phase 2: リアルワードの生成・保存(real_world_words テーブル)+楽曲詳細に表示
  - [ ] Phase 3: 使用回数の集計とプロンプトでの制限+検証リトライ
  - [ ] Phase 4: 天気(Open-Meteo)+ 設定画面に天気 ON/OFF・位置設定を追加
  - [ ] Phase 5: パラメータ一覧ページに使用ワードと残り回数を表示(任意)
