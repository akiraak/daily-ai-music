...
- [ ] iOSアプリデザインをする(色味はアイコンに合わせる。画面レイアウト・機能も再考)[plan](docs/plans/ios-app-design.md)
  - [ ] Phase 1: デザイン基盤(カラーセット ライト/ダーク + Theme.swift + 全画面へ適用)
  - [ ] Phase 2: ライブラリ刷新(今日の一曲ヒーロー・進行中ジョブカード・リスト行・日付グループ)
  - [ ] Phase 3: 楽曲詳細画面(歌詞・訳・狙い・リアルワードタグ・メタ情報・評価)
  - [ ] Phase 4: プレイヤー強化(ミニプレイヤー刷新・フルプレイヤー・連続再生・ロック画面 Now Playing)
  - [ ] Phase 5: 生成タブ再考(おまかせ生成 daily/run 主役化・カスタム生成折りたたみ・クレジット表示)
  - [ ] Phase 6: 設定の色味統一+仕上げ(芝生イコライザアニメ等)
  - [ ] Phase 7(任意): サーバー設定の閲覧・編集(GET/PUT /api/settings)
