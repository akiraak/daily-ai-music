...
- [] iOS 再生の復帰不全を直す(音楽が復帰しない: iPhone の画面を消してアプリに戻ると「再生中」表示のまま・電波が一度途切れて復活しても再生ができない)— [plan](docs/plans/ios-playback-recovery.md)
  - [ ] Phase 1: `isPlaying` を AVPlayer の実状態(`timeControlStatus` KVO)から導出 + ロック画面同期
  - [ ] Phase 2: 失敗した AVPlayerItem を再生操作で作り直す(位置復元付き)
  - [ ] Phase 3: オーディオセッション割り込み対応 + 実機での通し確認(機内モード・画面ロック・アラーム)
